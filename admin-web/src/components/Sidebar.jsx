...
const Sidebar = ({ onLogout, user, isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside className={`h-screen w-72 bg-white text-slate-700 flex flex-col fixed left-0 top-0 border-r border-slate-100 shadow-xl shadow-slate-100/50 z-50 transition-transform duration-300 ${
        ...
        <div className="p-8 flex items-center justify-between border-b border-slate-50">
          ...
          <button 
            onClick={onClose}
            className="lg:hidden p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-xl transition-colors cursor-pointer"
          >
            <X size={20} />
          </button>
        </div>
